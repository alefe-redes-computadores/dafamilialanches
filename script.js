/* =========================================================
   🚀 DFL v5.2.9f — VERSÃO FINAL ESTÁVEL COM CORREÇÃO DO BOTÃO CEP
   - CORREÇÃO CRÍTICA: Ajuste direto no listener do botão 'Buscar' (btn-calcular-frete) para garantir que o evento de clique seja capturado e a função buscarCEP seja chamada corretamente.
   - BLINDAGEM: Manutenção de todas as correções anteriores (frete por bairro, blindagem de inicialização).
========================================================= */
  ...
  /* ------------------ 🛒 MINI-CARRINHO ------------------ */
    ...
    async function getDynamicDeliveryFee(enderecoCompleto) {
        ...
        if (partes.length > 1) {
            ...
            // Se não houver hífen, assume a primeira parte
        ...
        if (nomeBairroBuscado.startsWith('qd')) nomeBairroBuscado = nomeBairroBuscado.substring(2).trim(); 
    ...
    */
    ...
    // --- CORREÇÃO DO LISTENER DE CEP (Substitua o bloco anterior) ---
    const btnCalcularFrete = document.getElementById('btn-calcular-frete');

    if (btnCalcularFrete) {
        btnCalcularFrete.addEventListener('click', safe((e) => {
            const cepInput = document.getElementById('cep-input');
            
            if (!cepInput) {
                console.error("ERRO: Campo do CEP não encontrado.");
                popupAdd("Erro interno. Tente recarregar a página.");
                return;
            }
