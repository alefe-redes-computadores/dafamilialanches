/* =========================================================  
   🚀 DFL v7.0 FINAL — SCRIPT COMPLETO E RESTAURADO
   - Restaurada a lógica de Recompensas que faltava
   - Correção definitiva de sintaxe e funcionalidades
========================================================= */  
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function setupAuthListener() {  
        auth.onAuthStateChanged(user => {  
            ...
            if (user) {  
                ...
                if (el.pedidosBtn) el.pedidosBtn.style.display = 'block'; if (el.recompensasBtn) el.recompensasBtn.style.display = 'block';
            } else {  
    ...
    function showPromoModal(promoId) { if (!el.promoModal || !PROMO_DATA[promoId]) return; currentPromoId = Number(promoId); const promo = PROMO_DATA[currentPromoId]; if (el.promoImg) el.promoImg.src = promo.img; if (el.promoTitle) el.promoTitle.textContent = promo.nome; if (el.promoPrice) { el.promoPrice.innerHTML = `<span class="old-price">De ${money(promo.precoAntigo)}</span> por <b>${money(promo.preco)}</b>`; } if (el.promoAddBtn) el.promoAddBtn.dataset.isCombo = "false"; Overlays.open(el.promoModal); }
    ...
    el.promoAddBtn?.addEventListener("click", (e) => { const promo = PROMO_DATA[currentPromoId]; if (!promo) return; addCommonItem(promo.nome, promo.preco); Overlays.closeAll(); });
