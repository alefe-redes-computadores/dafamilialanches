...
/* ------------------ 🛒 MINI-CARRINHO ------------------ */  
  ...
  /* --- FUNÇÃO FRETE DINÂMICO (v5.2 CORRIGIDA PARA UM ÚNICO DOCUMENTO) --- */  
  async function getDynamicDeliveryFee(localidade) {  
    const DELIVERY_FEE = DELIVERY_FEE_DEFAULT;   
    const TAXAS_DOC_ID = "taxas-unicas"; // <<-- ID do único documento onde estão todas as taxas  
      
    // 1. Normaliza a localidade vinda do ViaCEP ("Patos de Minas", etc.)  
    ...
    console.log("[FRETE] Localidade recebida para cálculo:", localidade, "=> normalizada:", localidadeClean);  
  
    // 2. Se ainda não temos cache, busca UMA VEZ só no Firestore  
    if (isFirebaseInitialized && !deliveryFeesCache) {  
      console.warn("[FRETE] Buscando taxas no Firestore (primeira vez)...");  
      try {  
        if (!db) throw new Error("Firestore não inicializado para buscar taxas.");  
          
        const docSnap = await db.collection("TaxasDeEntrega").doc(TAXAS_DOC_ID).get();  
        if (!docSnap.exists) {  
          console.warn("[FRETE] Documento de taxas não encontrado. Usando fallback R$ 6,00.");  
          return DELIVERY_FEE;  
        }  
        ...
        if (!Array.isArray(taxas) || taxas.length === 0) {  
          console.warn("[FRETE] Campo 'data' vazio ou inválido. Usando fallback R$ 6,00.");  
          return DELIVERY_FEE;  
        }  
  
        // Monta cache em formato { "patos-de-minas": 6, "bairro-x": 10, ... }  
        deliveryFeesCache = {};  
  
        taxas.forEach((entry) => {  
          if (!entry?.bairro || entry.valor == null) return;  
  
          const bairroNorm = String(entry.bairro)  
            .toLowerCase()  
            .normalize("NFD")  
            .replace(/[\u0300-\u036f]/g, "")  
            .trim();  
  
          const key = bairroNorm.replace(/\s+/g, "-");  
          deliveryFeesCache[key] = Number(entry.valor);  
        });  
  
        console.log("[FRETE] Cache de taxas montado:", deliveryFeesCache);  
      } catch (e) {  
        console.warn("[FRETE] Erro ao ler TaxasDeEntrega. Usando fallback R$ 6,00.", e);  
        return DELIVERY_FEE;  
      }  
    }  
  
    if (!deliveryFeesCache) {  
      console.warn("[FRETE] Cache não disponível. Usando fallback R$ 6,00.");  
      return DELIVERY_FEE;  
    }  
  
    // 3. Usa SEMPRE a chave 'patos-de-minas' (cidade)  
    const cidadeKey = "patos-de-minas";  
    let taxa = deliveryFeesCache[cidadeKey];  
  
    console.log("[FRETE] Tentando chave de cidade:", cidadeKey, "=> taxa encontrada:", taxa);  
  
    // 4. Se não tiver taxa específica da cidade, fallback  
    if (taxa == null || isNaN(taxa) || taxa < 0) {  
      console.warn("[FRETE] Cidade não mapeada ou valor inválido. Usando fallback R$ 6,00.");  
      taxa = DELIVERY_FEE;  
    }  
