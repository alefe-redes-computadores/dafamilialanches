/* =========================================================  
   🚀 DFL v5.3.1 — FINALIZADO E ESTÁVEL
   - Frete Manual (com lógica de extração e fallback)
   - Barra de Progresso Frete Grátis
   - Código limpo e à prova de quebras
========================================================= */  
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    /* ------------------ 🎯 ELEMENTOS ------------------ */  
    const el = {  
        // Essenciais do Carrinho/Header/Login
        ...
        // Banners e Promo (Mantidos para a lógica de inicialização)
        statusBanner: document.getElementById("status-banner"),  
        hoursBanner: document.querySelector(".hours-banner"),  
        promoTimer: document.getElementById("promo-timer"),
        
        // Outros elementos de navegação/painéis (Obrigatoriamente declarados para evitar erros)
        ...
        cPrev: document.querySelector(".c-prev"), cNext: document.querySelector(".c-next"), slides: document.querySelector(".slides"), 
        promoModal: document.getElementById("promo-modal"), promoImg: document.getElementById("promo-modal-img"), promoTitle: document.getElementById("promo-modal-title"), promoPrice: document.getElementById("promo-modal-price"), promoAddBtn: document.getElementById("promo-modal-add"), promoNavPrev: document.querySelector("#promo-modal .promo-nav.prev"), promoNavNext: document.querySelector("#promo-modal .promo-nav.next"), promoClose: document.querySelector("#promo-modal .promo-close"), 
    };
    ...
    renderMiniCart = function () {  
        ...
        bindMiniCartButtons();   
        enhanceMiniCartUI();  
        // Não é necessário chamar bindFreteManualEvents aqui, pois os elementos do modo manual são estáticos
    };
    ...
    // Removido o el.comboConfirm?.addEventListener("click", ...) para evitar quebra.
    ...
    function atualizarBarraProgresso() {
        ...
        if (subtotal >= LIMITE_FRETE_GRATIS) {
            ...
            // Não é necessário alterar o background se o CSS já está configurado, mas mantive a lógica de feedback
        } else if (falta <= 20) {
    ...
    function mostrarModoManual() {
        ...
        // Não renderiza, apenas muda o modo.
    }
    ...
    async function getDynamicDeliveryFee(enderecoCompleto) {
        ...
        for (const palavra of palavras) {
            ...
            for (const key in cacheAtual) {
                if (cacheAtual.hasOwnProperty(key) && key.includes(palavra)) { // CORRIGIDO: Checagem segura
    ...
    async function carregarRecompensas(userId) {  
        ...
        db.collection('Usuarios').doc(userId).onSnapshot(async doc => {  
            ...
            let progressoMsg = '';
            if (proximaRecompensa) { const faltam = proximaRecompensa.limite - feitos; progressoMsg = `Faltam ${faltam} pedidos para: ${proximaRecompensa.titulo || proximaRecompensa.valor}!`; } else { progressoMsg = '🎉 Parabéns! Todas as metas completas!'; }
            recompensasLista.innerHTML += `<p style="text-align:center;font-weight:600;">${progressoMsg}</p>`;
            const recompensasObtidas = RECOMPENSAS_DATA.filter(r => r.limite <= feitos); exibirRecompensas(feitos, recompensasObtidas, cupomStatus, RECOMPENSAS_DATA); 
            if (recompensasObtidas.length === 0) recompensasLista.innerHTML += `<p class="empty-orders" style="text-align:center;color:#666;margin-top:20px;">Faça pedidos para desbloquear.</p>`; 
            await carregarHistoricoRecompensas(userId);  
        }, error => { console.error("Erro contador:", error); recompensasLista.innerHTML = `<p class="empty-orders" style="color:red;">Erro ao ler progresso.</p>`; });  
    }  

    function exibirRecompensas(pedidosFeitos, recompensasDisponiveis, cupomStatus, RECOMPENSAS_DATA) {  
        const recompensasLista = document.querySelector(".rewards-list");
        if (!recompensasLista) return;  
        recompensasLista.innerHTML += (recompensasDisponiveis || []).map(r => {  
        ...
        document.querySelectorAll('.recompensa-aplicar-btn').forEach(btn => { btn.addEventListener('click', (e) => { const codigo = e.currentTarget.dataset.cupom; if (codigo) { couponApplied = codigo; localStorage.setItem("dflCoupon", couponApplied); document.getElementById("coupon-input").value = codigo; renderMiniCart(); Overlays.closeAll(); popupAdd(`Cupom ${codigo} aplicado! ✅`); Overlays.open(el.miniCart); } }); });  
    }  

    async function carregarHistoricoRecompensas(userId) {  
        const historicoLista = document.querySelector(".rewards-list");
        if (!historicoLista) return; historicoLista.innerHTML += `<p style="margin-top:15px; font-weight: bold;">Histórico de Recompensas:</p>`;
...
window.onload = function() {
    ...
    // Ligar Botoes de Navegação e Painéis (Admin desativado)
    document.getElementById('reports-btn')?.addEventListener("click", () => { alert("Recurso de Relatórios Desativado."); });
    document.querySelector('.c-prev')?.addEventListener("click", () => { alert("Carrossel em desenvolvimento."); });
    document.querySelector('.c-next')?.addEventListener("click", () => { alert("Carrossel em desenvolvimento."); });

    // Ligar eventos dos painéis laterais
    const pedidosBtn = document.querySelector('.meus-pedidos-btn');
    const recompensasBtn = document.querySelector('.recompensas-btn');
    if (pedidosBtn) pedidosBtn.addEventListener("click", () => { if (!currentUser) { alert("Faça login!"); Overlays.open(el.loginModal); return; } Overlays.open(el.pedidosPanel); carregarPedidos(currentUser.uid); });
    if (recompensasBtn) recompensasBtn.addEventListener("click", () => { if (!currentUser) { alert("Faça login!"); Overlays.open(el.loginModal); return; } Overlays.open(el.recompensasPanel); carregarRecompensas(currentUser.uid); });
};
