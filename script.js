/* =========================================================  
   🚀 DFL v8.2 FINAL — SCRIPT INTEGRAL (CORRIGIDO)
   - Recompensas: Erro 'not defined' resolvido (funções movidas para o topo)
   - Admin: Botão de relatórios restaurado
   - Finalizar: Link WhatsApp corrigido
========================================================= */  

document.addEventListener("DOMContentLoaded", () => {

    // ============================================================
    // 0. VARIÁVEIS GLOBAIS E FUNÇÕES UTILITÁRIAS (SCOPE FIX)
    // ============================================================
    ...
    // Função Recompensas (Declarada antes do uso para evitar erro)
    ...
    /* ============================================================
       🔥 DADOS DAS PROMOÇÕES (REORDENADOS)
    ============================================================ */
    const PROMO_DATA = [  
        null,   
    ...
    function filtrarCards(query) {
        ...
        document.querySelectorAll('.card').forEach(card => {
            ...
            let match = false;
            if (nomeNorm.includes(queryNorm)) match = true;
            else {
    ...
    const Backdrop = { show() { el.cartBackdrop.classList.add("active"); document.body.classList.add("no-scroll"); }, hide() { el.cartBackdrop.classList.remove("active"); document.body.classList.remove("no-scroll"); if(el.pedidosPanel) el.pedidosPanel.classList.remove("active"); if(el.recompensasPanel) el.recompensasPanel.classList.remove("active"); } };
    const Overlays = { closeAll() { document.querySelectorAll(".modal.show, #mini-cart.active, .pedidos-panel.active, .recompensas-panel.active, #admin-dashboard.show").forEach((e) => e.classList.remove("show", "active")); Backdrop.hide(); }, open(modalLike) { Overlays.closeAll(); if (!modalLike) return; modalLike.classList.add((modalLike.id === "mini-cart" || modalLike.id === "painelPedidos" || modalLike.id === "recompensas-panel") ? "active" : "show"); Backdrop.show(); } };  
    ...
    function mostrarPopupRecompensa(msg) { let pop = document.getElementById("conquista-popup"); if (!pop) { pop = document.createElement("div"); pop.id = "conquista-popup"; pop.style.cssText = `position:fixed;bottom:120px;left:50%;transform:translateX(-50%) scale(0);background:#4CAF50;color:white;padding:15px 25px;border-radius:12px;font-weight:bold;text-align:center;box-shadow:0 4px 15px rgba(0,0,0,0.3);z-index:10001;opacity:0;transition:transform 0.4s cubic-bezier(0.175,0.885,0.32,1.275),opacity 0.4s;`; document.body.appendChild(pop); } pop.textContent = msg; pop.style.opacity = '1'; pop.style.transform = 'translateX(-50%) scale(1)'; setTimeout(() => { pop.style.transform = 'translateX(-50%) scale(0)'; pop.style.opacity = '0'; }, 6000); }
    ...
    function renderMiniCart() {  
        ...
        el.miniList.innerHTML = cart.map((item, idx) => ` <div class="cart-item" style="border-bottom:1px solid #eee;padding:8px 0;"> <div style="display:flex;justify-content:space-between;align-items:center;"> <div style="flex:1;"><p style="font-weight:600;margin-bottom:2px;">${item.nome}</p><p style="color:#666;font-size:0.85rem;">${money(item.preco)} × ${item.qtd}</p></div> <div style="display:flex;gap:8px;align-items:center;"> <button type="button" class="cart-minus" data-idx="${idx}" style="background:#ff4081;color:#fff;border:none;border-radius:5px;width:24px;height:24px;cursor:pointer;">−</button><span style="font-weight:600;min-width:20px;text-align:center;">${item.qtd}</span><button type="button" class="cart-plus" data-idx="${idx}" style="background:#4caf50;color:#fff;border:none;border-radius:5px;width:24px;height:24px;cursor:pointer;">+</button><button type="button" class="cart-remove" data-idx="${idx}" style="background:#d32f2f;color:#fff;border:none;border-radius:5px;width:24px;height:24px;cursor:pointer;">🗑</button> </div></div></div>`).join("");  
    }  
    ...
    function showPromoModal(promoId) { if (!el.promoModal || !PROMO_DATA[promoId]) return; currentPromoId = Number(promoId); const promo = PROMO_DATA.find(p => p && p.id === currentPromoId); if (el.promoImg) el.promoImg.src = promo.img; if (el.promoTitle) el.promoTitle.textContent = promo.nome; if (el.promoPrice) { el.promoPrice.innerHTML = `<span class="old-price">De ${money(promo.precoAntigo)}</span> por <b>${money(promo.preco)}</b>`; } if (el.promoAddBtn) el.promoAddBtn.dataset.isCombo = "false"; Overlays.open(el.promoModal); }
    ...
    el.promoAddBtn?.addEventListener("click", (e) => { const promo = PROMO_DATA.find(p => p && p.id === currentPromoId); if (!promo) return; addCommonItem(promo.nome, promo.preco); Overlays.closeAll(); });
    el.promoNavPrev?.addEventListener("click", () => { let newId = currentPromoId - 1; if (newId < 1) newId = 10; showPromoModal(newId); });
    el.promoNavNext?.addEventListener("click", () => { let newId = currentPromoId + 1; if (newId > 10) newId = 1; showPromoModal(newId); });
    ...
    el.recompensasBtn?.addEventListener("click", () => { if (!currentUser) { alert("Faça login para ver suas recompensas!"); Overlays.open(el.loginModal); return; } Overlays.open(el.recompensasPanel); carregarRecompensas(currentUser.uid); });
    ...
    if (cookieBanner && cookieAcceptBtn) { if (localStorage.getItem("dfl-cookies-accepted") === "true") cookieBanner.style.display = "none"; else cookieBanner.classList.add("show"); cookieAcceptBtn.addEventListener("click", () => { localStorage.setItem("dfl-cookies-accepted", "true"); cookieBanner.classList.remove("show"); setTimeout(() => cookieBanner.style.display = "none", 500); }); }

    console.log("%c🔥 DFL v8.2 — SCRIPT INTEGRAL CORRIGIDO", "background:#4CAF50;color:#fff;padding:5px;border-radius:5px;");  
