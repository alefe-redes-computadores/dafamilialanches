  ...
  */
  ...
  function inicializarFirebase() {
      ...
      try {
          ...
          // 🚨 ADIÇÃO V3.7.0: Inicializa o módulo de Frete (após Firebase e DOM pronto)
          if (window.DFL_Frete) {
              DFL_Frete.init();
          }
  ...
  el.extrasConfirm?.addEventListener("click", () => {
    ...
    checks.forEach(c => {
      ...
      if (extrasContagem[adicional.nome]) {
        ...
        extrasContagem[adicionais[idx].nome] = { preco: adicional.preco, qtd: 1 };
      }
    });
...
/* ------------------ ⚙️ CONFIGURAÇÕES V3.0 (MANTIDO) ------------------ */
  ...
  */
  async function calcTotals() {
    ...
    // 🚨 PONTO DE INTEGRAÇÃO V3.7.0: Obter Frete do novo Módulo
    let calculatedDelivery = DELIVERY_FEE;
    let isFreteModuleActive = false;

    if (window.DFL_FLAGS?.freightEnabled && window.DFL_Frete?.state?.lastEstimate) {
      calculatedDelivery = window.DFL_Frete.state.lastEstimate.fee;
      isFreteModuleActive = true;
    }

    // 'd' (discountInfo) agora vem do Firestore e precisa de 'await'
    const d = await validarCupomFirestore(couponApplied, subtotal); 
    
    // Se o módulo de frete está ativo, ele define a taxa base.
    // O cupom 'frete' zera essa taxa.
    const baseDelivery = isFreteModuleActive ? calculatedDelivery : DELIVERY_FEE;

    const delivery = d.freeShipping ? 0 : baseDelivery;
  ...
  */
  async function fecharPedido() {
    ...
    // 🚨 PONTO DE INTEGRAÇÃO V3.7.0: Salvar informações da estimativa de frete
    let freightEstimateData = {};
    if (window.DFL_Frete?.state?.lastEstimate) {
        freightEstimateData = {
            freightFee: DFL_Frete.state.lastEstimate.fee,
            freightSource: DFL_Frete.state.lastEstimate.source,
            freightZone: DFL_Frete.state.lastEstimate.zoneName,
            freightEta: DFL_Frete.state.lastEstimate.eta,
        };
    }
    ...
    const pedido = {
      ...
      ...freightEstimateData // Integra os dados do frete (se existirem)
    };
...
// =========================================================
// 🧩 Módulo de Frete — Esqueleto (DFL_Frete)
// =========================================================
(function(){
  "use strict";

  // Evita reinicialização dupla
  if (window.__DFL_FRETE_INIT__) { LOG.info("Frete já inicializado."); return; }
  window.__DFL_FRETE_INIT__ = true;

  const Frete = {
    state: {
      mode: "bairro",   // "bairro" | "cep_range"
      zones: [],        // Tabela de zonas (carregada do Firestore)
      overrides: [],    // Regras específicas (opcional)
      lastEstimate: null,
    },

    // 🔌 Carrega zonas do Firestore (coleção: shipping_zones)
    async loadZonesFromFirestore(){
      try{
        if (!window.firebase?.firestore) { LOG.warn("Firestore indisponível (lazy) — adiar carregamento de frete."); return; }
        const db = firebase.firestore();
        const snap = await db.collection("shipping_zones").where("active","==",true).get();
        this.state.zones = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        LOG.info("Zonas de frete carregadas:", this.state.zones.length);
      }catch(e){ LOG.error("Falha ao carregar zonas de frete:", e); }
    },

    // 🧮 Estima frete por bairro ou faixa de CEP (cep: string, bairro: string)
    estimate({ cep = "", bairro = "" } = {}){
      // ⚠️ Placeholder de lógica (será implementado no v3.7.1+)
      // Estratégias:
      // - mode === "bairro": busca zona por nome (case-insensitive)
      // - mode === "cep_range": normaliza CEP (somente dígitos) e verifica em ranges [from..to]
      // - aplica overrides (frete grátis por valor mínimo, horários, etc.)
      const fee = 0;  // manter zero até lógica real
      const eta = "30–60 min";
      const zoneName = bairro || "Zona Padrão";
      const result = { fee, eta, zoneName, source: "placeholder" };
      this.state.lastEstimate = result;
      LOG.info("Estimativa (placeholder):", result);
      return result;
    },

    // 🔁 Integração com UI: mini‑cart / checkout
    bindUI(){
      // Captura campos se existirem (não quebra se ausentes)
      const cepInput    = document.querySelector("#checkout-cep, input[name='cep']");
      const bairroInput = document.querySelector("#checkout-bairro, input[name='bairro']");
      const outText     = document.querySelector("#frete-estimativa"); // exibir texto (ETA/valor)
      const totalHook   = document.querySelector("#cart-total, .cart-total"); // onde somar o frete

      const run = () => {
        if (!window.DFL_FLAGS?.freightEnabled) return;
        const cep    = cepInput?.value?.trim() || "";
        const bairro = bairroInput?.value?.trim() || "";
        const est = this.estimate({ cep, bairro });
        if (outText){
          outText.textContent = est.fee > 0
            ? `Frete: R$ ${Number(est.fee).toFixed(2).replace(".", ",")} • ${est.eta} • ${est.zoneName}`
            : `Frete a confirmar • ${est.eta}`;
        }
        // Integração com total do carrinho (não comita nada ainda)
        try {
          if (totalHook && est.fee > 0) {
            // Exemplo: se houver função global de updateTotal, use-a; caso contrário, apenas log
            if (typeof window.__applyFreightToCartTotal === "function") {
              window.__applyFreightToCartTotal(est.fee);
            } else {
              LOG.info("Total do carrinho deve somar frete via função existente (__applyFreightToCartTotal).");
            }
          }
        } catch (e) { LOG.error("Falha ao aplicar frete ao total:", e); }
      };

      // Eventos
      cepInput    && cepInput.addEventListener("change", run);
      bairroInput && bairroInput.addEventListener("change", run);

      // Primeiro disparo (se já houver valor digitado)
      run();
      LOG.info("UI de frete vinculada.");
    },

    // 🔁 Inicialização (chamar após Lazy Firebase e render do DOM)
    async init(){
      LOG.info("Inicializando módulo de frete (esqueleto)…");
      // Esta chamada se beneficia do Lazy Load do Firebase (chamado em inicializarFirebase)
      await this.loadZonesFromFirestore(); 
      this.bindUI();
      LOG.info("Módulo de frete pronto (modo esqueleto).");
    },
  };

  // Expor namespace (debug)
  window.DFL_Frete = Frete;

  // Não precisamos mais do window.addEventListener("load") porque o init será chamado DENTRO do bloco DOMContentLoaded (linha ~460) após o Firebase iniciar.
  
})();
// ===== Fim do Módulo de Frete (v3.7.0) =====