...
document.addEventListener("DOMContentLoaded", () => {
    // MÁSCARA AUTOMÁTICA DO CEP (v5.3)
...
async function getDynamicDeliveryFee(enderecoCompleto) {
    const DELIVERY_FEE = 6.00; // usa o mesmo valor padrão
    ...
    try {
        // Remove a parte da cidade/UF entre parênteses, se existir
        const partePrincipal = enderecoCompleto.split("(")[0].trim();

        // Ex.: "Rua Netuno - Jardim Andrades"
        const partes = partePrincipal.split(" - ");

        // 👉 CORREÇÃO IMPORTANTE:
        // Em vez de pegar SEMPRE o índice 1 (que quebrava em alguns formatos),
        // usamos SEMPRE o ÚLTIMO trecho como bairro.
        //
        // Ex:
        //  - "Rua X - Jardim Andrades"               → bairro = "Jardim Andrades"
        //  - "Rua X - Bloco B - Jardim Andrades"     → bairro = "Jardim Andrades"
        //  - "Rua X"                                 → bairro = "Rua X" (fallback)
        if (partes.length >= 2) {
    ...
    try {
        if (window.firebase && !window.firebase.apps?.length) {
            // nada – inicialização é feita em inicializarFirebase()
        }

        if (typeof db === "undefined" || !db) {
            console.warn("FW: Firestore não está disponível no momento. Fallback.");
            return DELIVERY_FEE;
        }
    ...
    if (!Object.keys(cacheAtual).length) return DELIVERY_FEE;

    // 5. BUSCA EXATA PELO BAIRRO
    if (cacheAtual[bairroClean] !== undefined) {
        console.log(`FW: Match Exato para ${bairroClean}. Taxa: ${cacheAtual[bairroClean]}`);
    ...
    for (const palavra of palavras) {
        ...
        for (const key in cacheAtual) {
            if (key.includes(palavra)) {
                console.log(`FW: Match Palavra-Chave '${palavra}' em ${key}. Taxa: ${cacheAtual[key]}`);
