// DFL v1.4.1 — hotfix “nada clica” (JS completo)
// Reativa: carrossel, contador, carrinho, extras, combos (bebida), login e “Meus Pedidos”
...
/* =========================================
   🧩 GARANTIR ELEMENTOS QUE O HTML PODE NÃO TER
   (criamos quando não existir)
========================================= */
// Mini-carrinho (HTML já tem; só conferimos X do topo)
const miniCart = document.getElementById("mini-cart");
const cartBackdrop = document.getElementById("cart-backdrop");
const cartCount = document.getElementById("cart-count");

// Modal de “Adicionais” (lanches)
let extrasModal = document.getElementById("extras-modal");
if (!extrasModal) {
  extrasModal = document.createElement("div");
  extrasModal.id = "extras-modal";
  extrasModal.className = "modal";
...
if (!loginModal) {
  ...
  loginModal.innerHTML = `
    <div class="modal-content login-box">
      <button class="login-x" title="Fechar">✖</button>
      <h3 style="margin-bottom:8px;">Entrar / Cadastrar</h3>
...
if (!userBtn){
  ...
  userBtn.id="user-btn"; userBtn.className="user-button"; userBtn.textContent="Entrar / Cadastrar";
  document.querySelector(".header")?.appendChild(userBtn);
}
userBtn.addEventListener("click", ()=>{ loginModal.classList.add("show"); document.body.classList.add("no-scroll"); });
...
document.querySelector(".btn-primario")?.addEventListener("click", ()=>{
  ...
  auth.signInWithEmailAndPassword(email, senha)
    .then(cred=>{ currentUser=cred.user; userBtn.textContent=`Olá, ${currentUser.email.split("@")[0]}`; closeModal(loginModal); showOrdersFabIfLogged(); })
    .catch(()=> auth.createUserWithEmailAndPassword(email, senha).then(cred=>{
      currentUser=cred.user; userBtn.textContent=`Olá, ${currentUser.email.split("@")[0]}`; closeModal(loginModal); alert("Conta criada com sucesso! 🎉"); showOrdersFabIfLogged();
...
document.querySelector(".btn-google")?.addEventListener("click", ()=>{
  ...
  auth.signInWithPopup(provider).then(result=>{
    currentUser=result.user; userBtn.textContent=`Olá, ${currentUser.displayName?.split(" ")[0]||"Cliente"}`; closeModal(loginModal); showOrdersFabIfLogged();
...
function fecharPedido(){
  ...
  if (!currentUser){ alert("Você precisa estar logado para enviar o pedido!"); loginModal.classList.add("show"); return; }
...
ordersFab.addEventListener("click", ()=>{
  ...
  ordersPanel.classList.add("active"); carregarPedidosSeguro();
});
...
console.log("✅ DFL 1.4.1 hotfix carregado");
});