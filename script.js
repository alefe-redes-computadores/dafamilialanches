/* =========================================================
🚀 DFL v4.1 — CORREÇÃO DE ÍCONES (CONTÉM "OURO" ETC.)

Correção: getTierIcon agora detecta "Cliente Ouro".

Exibição de Cupom forçada para todos os tipos.
========================================================= */
...
/* --- 🆕 FUNÇÃO HELPER DE ÍCONES (CORRIGIDA: DETECÇÃO POR TRECHO) --- */
...
return '👤'; // Padrão se não achar nível
}
...
try {  
  ...
  if (recompensaAtingida) {  
      ...
      // CORREÇÃO AQUI: Detecta se é "Cliente Ouro" e passa para função  
...
/* =========================================================
🎁 MINHAS RECOMPENSAS (ATUALIZADO v4.1: DETECÇÃO DE TEXTO)
========================================================= */
...
const recompensasHtml = recompensasDisponiveis.map(r => {  
    ...
    // --- ÍCONES EMOJI (CORRIGIDO v4.1: DETECÇÃO POR TRECHO) ---  
    let icon = '🎁';  
    ...
    // --- EXIBIÇÃO DE CUPOM (CORRIGIDO v4.1: Mostra cupom se tiver valor, independente do tipo) ---  
...
if (snapshot.empty) {  
    ...
    const historicoHtml = logs.map(log => {  
        ...
        // --- ÍCONES EMOJI (LÓGICA ATUALIZADA v4.1: DETECÇÃO POR TRECHO) ---  
        let icon = '🎁';  
        if (log.titulo && (log.titulo.toLowerCase().includes('ouro') || log.titulo.toLowerCase().includes('platina'))) {  
...
console.log("%c🔥 DFL v4.1 — Ícones Inteligentes + Segurança", "background:#4CAF50;color:#fff;padding:5px;border-radius:5px;");
