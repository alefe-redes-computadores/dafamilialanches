...
function popupAdd(msg = "Ação realizada!") {
  // Garante elemento de toast
  let el = document.querySelector('.popup-add');
  if (!el) {
    el = document.createElement('div');
    el.className = 'popup-add';
    // Estilos inline mínimos (não dependem do CSS externo)
...
// Deriva thumbnail a partir do primeiro item do pedido (fallback local)
function derivePedidoThumb(p) {
  try {
    // Se já existir no documento, respeita
    if (p && typeof p.thumb === 'string' && p.thumb.trim()) return p.thumb;
    // Usa primeiro item para inferir
    const first = Array.isArray(p?.itens) && p.itens.length ? p.itens[0] : null;
    const name = (first && (first.nome || first.name || first.title || first.id || "")) + "";
    const n = name.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
    // Mapeamento simples -> arquivo em /imagens/
    const map = {
      "bao": "bao.png",
      "uai": "uai.png",
      "trem": "trem.png",
      ...
      "trembao": "trembao.png",
      // Combos comuns
      "combo casal tradicional": "combo1.png",
      "combo casal artesanal": "combo2.png",
      "combo familia tradicional": "combo3.png",
      "combo familia artesanal": "combo4.png"
    };
    // Tenta correspondência por chave incluída no nome
    for (const key in map) {
      if (n.includes(key)) return "/imagens/" + map[key];
    }
    // Fallback seguro
