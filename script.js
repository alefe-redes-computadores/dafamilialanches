/* =========================================================
   🍔 DFL v2.9 — Repetir Pedido + Histórico Visual
   - Base completa e compatível com v2.8 Estável
   - Nenhuma função removida, apenas estendida
========================================================= */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* 🚨 NOVO BANCO DE DADOS (mantido da v2.8) */
  const PROMO_DATA = [
    null,
  ...
  const Overlays = {
    closeAll() {
      document.querySelectorAll(".modal.show, #mini-cart.active, .orders-panel.active, #admin-dashboard.show")
  ...
  /* ------------------ 🔥 FIREBASE (mantido, sem alteração estrutural) ------------------ */
  ...
  try {
    if (!window.firebase) throw new Error("Biblioteca principal do Firebase (app) não carregou.");
    if (!firebase.apps.length) firebase.initializeApp(firebaseConfig);
    if (!firebase.auth) throw new Error("Módulo de Autenticação (auth) não carregou.");
    auth = firebase.auth();
    if (!firebase.firestore) throw new Error("Módulo Firestore não carregou.");
    ...
    document.body.innerHTML = `
      <div style="padding:20px;text-align:center;font-size:1.2rem;color:red;font-family:sans-serif;margin-top:50px;">
        ...
        <small>Verifique sua conexão e recarregue a página.</small><br><br>
        <small style="color:#666">Detalhe: ${error.message}</small>
      </div>`;
    return;
  }
/* === DFL v2.9 - Parte 2/3 ===
   🔐 Login + 📦 Meus Pedidos + 🔁 Repetir Pedido
   (Totalmente compatível com Firestore e a base anterior)
========================================================= */
  ...
  /* =========================================================
     📦 MEUS PEDIDOS (Atualizado v2.9)
     - Histórico visual com miniaturas
  ...
  function repetirPedido(pedido) {
    ...
    pedido.itens.forEach((itemStr) => {
      const [nomeParte, qtdParte] = itemStr.split(" x");
      const nome = nomeParte.trim();
      const qtd = parseInt(qtdParte || "1", 10);

      // Busca o preço médio aproximado do item anterior
      let preco = 0;
      const refItem = PROMO_DATA.find(p => p && nome.includes(p.nome));
      if (refItem) preco = refItem.preco;
      else {
        // Fallback simples: tenta encontrar o preço em cache do último pedido
        preco = (pedido.total / pedido.itens.length) / (qtd || 1);
      }
    ...
    renderMiniCart();
    popupAdd(`🔁 ${adicionados} item(ns) adicionados novamente!`);
    Overlays.open(el.miniCart);
  }

  /* === Função: Carregar pedidos com histórico visual === */
  function carregarPedidosSeguro() {
    ...
    const render = (snap) => {
      ...
      pedidos.forEach((p) => {
        ...
        const mini = itens.length ? itens[0] : "";
        ...
        const miniatura = (() => {
          const promoMatch = PROMO_DATA.find(d => d && mini.includes(d.nome));
          return promoMatch ? promoMatch.img : "imagens/padrao.png";
        })();
        ...
        box.innerHTML = `
          <div class="order-header">
            <img src="${miniatura}" alt="Pedido" class="order-thumb"/>
            <div class="order-info">
              <h4>📅 ${dataFormatada}</h4>
              <p style="margin:2px 0 6px;"><b>Total:</b> ${money(p.total || 0)}</p>
              ${p.cupom ? `<p style="font-size:0.8rem;color:#666;">Cupom usado: ${p.cupom}</p>` : ""}
            </div>
          </div>

          <div class="order-body">
            <p style="margin:6px 0;"><b>Itens:</b><br>• ${itens.join("<br>• ")}</p>
            ${p.endereco ? `<p style="margin:6px 0;"><b>Endereço:</b> ${p.endereco}</p>` : ""}
          </div>

          <div class="order-actions">
            <button class="btn-repeat" type="button" data-id="${p.id}">🔁 Repetir Pedido</button>
          </div>
        `;
      ...
      // Vincula os botões de repetir
      container.querySelectorAll(".btn-repeat").forEach(btn => {
        btn.addEventListener("click", () => {
          const id = btn.dataset.id;
          const pedido = pedidos.find(pp => pp.id === id);
...
/* === DFL v2.9 - Parte 3/3 ===
   📊 Dashboard Admin + 🍪 Cookies + UX Final + Segurança
========================================================= */

/* =========================================================
   📊 ADMIN DASHBOARD (Mantido, com otimizações leves)
========================================================= */
const ADMINS = [
...
function gerarResumoECharts(pedidos) {
  ...
  const labelsPedidos = Object.keys(pedidosPorDia).sort((a,b)=>{
    ...
    return new Date(`${mA}/${dA}/2025`) - new Date(`${mB}/${dB}/2025`);
  });
  const dataPedidos = labelsPedidos.map(l => pedidosPorDia[l]);
  ...
  chartPedidos = new Chart(ctxPedidos, {
    ...
    options: { responsive: true, plugins: { title: { display: true, text: 'Volume de Pedidos' } } }
  });
  ...
  const top10 = Object.entries(produtosContagem).sort(([,a],[,b])=>b-a).slice(0,10);
  const labelsProdutos = top10.map(p=>p[0]);
  const dataProdutos = top10.map(p=>p[1]);
  ...
  chartProdutos = new Chart(ctxProdutos, {
    ...
    options: { indexAxis: 'y', responsive: true, plugins: { title: { display: true, text: 'Top 10 Itens' } } }
  });
}
    ...
    .then(snap => {
      const pedidos = snap.docs.map(d => {
        ...
        return {
          ...p,
          id: d.id,
          subtotal, entrega, desconto, total,
          data: typeof p.data === "string" ? new Date(p.data) :
...
/* =========================================================
   🔐 Segurança/Admin + 🍪 Cookies + UX Final
========================================================= */
...
/* =========================================================
   🧩 Fechar modais clicando fora (mantido da v2.8)
========================================================= */
...
window.addEventListener("error", (e) => {
  if (String(e?.message || "").toLowerCase().includes("split")) {
    popupAdd("Houve um pequeno erro ao ler dados. Atualize a página.");
  }
  console.warn("⚠️ Erro interceptado:", e?.message);
});

console.log("%c🔥 DFL v2.9 — Repetir Pedido + Histórico Visual OK — Base 2.8 Estável",
            "background:#ffb300;color:#fff;padding:8px 12px;border-radius:8px;font-weight:700;");
