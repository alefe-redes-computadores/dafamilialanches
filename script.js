/* =========================================================
   🚀 DFL v3.8.0 — CORREÇÃO DO LOGIN GOOGLE + OTIMIZAÇÕES
   - Corrige erro "operation cancelled due to another conflicting popup"
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let isLoginInProgress = false; // NOVO: Flag para prevenir múltiplos popups
  ...
  function renderMiniCart() {
    try {
      ...
      if (Array.isArray(cart) ? cart.length === 0 : (!cart || (cart.items && cart.items.length === 0))) {
        if (listEl) {
          listEl.innerHTML = `
            <div class="cart-empty-msg" style="
              ...
              line-height: 1.4;
            ">
              <div style="font-size:1.05rem; margin-bottom:6px; color:#222;">🍔 Carrinho vazio por aqui…</div>
        ...
        if (footEl) footEl.style.opacity = "0.85";
      }
    } catch(e) { 
      try { console.warn('Empty-cart banner guard', e); } catch(_){} 
    }
  ...
  const handleLoginSuccess = (user) => {
    currentUser = user;
    isLoginInProgress = false; // NOVO: Libera flag
    popupAdd("Login realizado com sucesso!");
    Overlays.closeAll();
  };

  const handleLoginError = (err) => {
    isLoginInProgress = false; // NOVO: Libera flag em caso de erro
  ...
  // 🚨 CORREÇÃO PRINCIPAL: Previne múltiplos popups do Google
  el.googleBtn?.addEventListener("click", () => {
    ...
    isLoginInProgress = true; // NOVO: Bloqueia novos popups
...
/* ------------------ ➕ Adicionais ------------------ */
  ...
  /* ------------------ 🎁 Carregar Configurações de Recompensas ------------------ */
...
console.log("%c🔥 DFL v3.8.0 — Login Google Corrigido ✅",
...
/* =========================================================
PATCH v3.8.0 — Delegação de Fechar Modais
========================================================= */
(function () {
...
/* =========================================================
PATCH EXTRA — Remove backdrop ao fechar
========================================================= */
